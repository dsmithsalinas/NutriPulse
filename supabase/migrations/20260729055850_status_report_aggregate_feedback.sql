-- Keep the daily status report aggregate-only by replacing raw feedback text
-- with counts grouped by category.
  ...
  SELECT jsonb_build_object(
    ...
    'feedback_new',
      (SELECT COALESCE(jsonb_agg(jsonb_build_object(
                ...
                'count',    grouped.count) ORDER BY grouped.category), '[]'::jsonb)
        FROM (
